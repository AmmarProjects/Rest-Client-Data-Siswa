(function () {
    $.getJSON('http://kelompoksister.000webhostapp.com/api/CTF?kunci=asiyap', function (data) {
        var source = data;
        localStorage.setItem('local', JSON.stringify(source));
        var stored = JSON.parse(localStorage.getItem('local'));

        var allQuestions = [{
            question: stored.data[ 1 ].soal,
        ...
        function createElement(index) {
            ...
            var header = $('<h6>Question No. ' + (index + 1) + ' :</h6>');
            element.append(header);

            var question = $('<h4 class="mt-2 font-weight-bold">').append(allQuestions[index].question);
        ...
        function radioButtons(index) {
            var radioItems = $('<ul class="mt-3">');
            var item;
            var input = '';
            for (var i = 0; i < allQuestions[index].options.length; i++) {
                item = $('<ol type="A" class="ml-4 mt-4">');
